...
Calls without custom configuration use a bounded input-string cache. Configured
calls are parsed against the supplied tokens/plugins and are not stored in the
global cache.
...
interface ParsedResult {
  ...
  /** Scroll-linked output ranges */
  scroll: ScrollConfig;

  /** Named variants and selected lifecycle states */
  variants: VariantMap;
  variantState: VariantState;

  /** Actionable parser and extension diagnostics */
...
A dictionary of CSS/transform properties to their target values. For example, `{ scale: 1.1, opacity: 1 }`, `{ x: 20, rotate: 45 }`, or `{ scale: [1, 1.2, 1] }` for keyframe arrays.
...
interface DragConfig {
  ...
  dragConstraints?: {
    ...
    right?: number;
    bottom?: number;
  }; // animate-drag-constraint-{edge}-{n}
}
```
...
The build-time transform handles static literals and statically discoverable
`animate-*` tokens inside template literals. If the animation token or value is
constructed entirely at runtime, use `mw.*`; it calls `parseMotionClasses` at
runtime and passes the result to the underlying `motion.*` component.
  ...
  return (
    <mw.div
      className={`p-6 rounded-xl animate-hover:scale-105 animate-duration-200 ${
        isActive
          ? "bg-blue-500 animate-enter:opacity-100"
          : "bg-gray-800 animate-enter:opacity-50"
      }`}
    >
      Card content
    </mw.div>
...
- Unconfigured results use the bounded className cache; configured runtime
  components parse against their provider configuration.
...
Create a typed runtime wrapper for a design-system component. The component must forward its ref and `className` to the underlying element.

```tsx
const MotionButton = mw.create(Button);

<MotionButton className="animate-tap:scale-95 animate-spring">
  Save
</MotionButton>;
```

Lowercase JSX elements continue to use the compile-time transform; `mw.create` is an explicit runtime bridge for custom components.

### `MotionwindProvider`

Provide project configuration to runtime `mw.*` and `mw.create` components while applying the selected reduced-motion policy through Motion:
...
## `withMotionwind(nextConfig, motionwindConfig)`

A Next.js configuration wrapper that adds the Motionwind pre-transform to both
...
2. Static animation tokens are transformed on host elements and supported
...
export default defineConfig({
  plugins: [
    motionwind(motionwindConfig), // Must come before the React plugin
    react(),
  ],
});
```
...
function motionwind(config?: MotionwindConfig): VitePlugin;
```
...
1. **Static animation tokens are transformed.** This includes literals and
   statically discoverable tokens inside template literals. Fully computed
   animation tokens use `mw.*`.
2. **Lowercase host tags are the primary path.** Simple uppercase component
   identifiers use Motion's component bridge; `mw.create(Component)` is the
   explicit typed API for design-system components. Member expressions are not
   rewritten.
