...
function serializeValue(v: unknown): string {
  ...
  if (typeof v === "object" && v !== null) return serializeObject(v as Record<string, unknown>);
  return String(v);
}
...
export function generateMotionCode(
  ...
  const parsed: ParsedResult = parseMotionClasses(className);
  ...
  const attrs: string[] = [];
  if (parsed.tailwindClasses) attrs.push(`className="${parsed.tailwindClasses}"`);
  ...
  for (const [gestureKey, vals] of Object.entries(parsed.gestures)) {
    ...
    attrs.push(`${gestureKey}={${serializeObject(vals as Record<string, unknown>)}}`);
  }

  if (Object.keys(parsed.transition).length) {
    attrs.push(`transition={${serializeObject(parsed.transition as Record<string, unknown>)}}`);
  }
  if (Object.keys(parsed.viewport).length) {
    attrs.push(`viewport={${serializeObject(parsed.viewport as Record<string, unknown>)}}`);
  }

  // Drag
  if (parsed.dragConfig.drag === true) attrs.push(`drag`);
  else if (parsed.dragConfig.drag) attrs.push(`drag="${parsed.dragConfig.drag}"`);
  ...
  if (parsed.dragConfig.dragMomentum === false) attrs.push(`dragMomentum={false}`);
  if (parsed.dragConfig.dragDirectionLock) attrs.push(`dragDirectionLock`);
  if (parsed.dragConfig.dragConstraints)
    attrs.push(`dragConstraints={${serializeObject(parsed.dragConfig.dragConstraints)}}`);

  // Layout
  if (parsed.layoutConfig.layout === true) attrs.push(`layout`);
  else if (parsed.layoutConfig.layout) attrs.push(`layout="${parsed.layoutConfig.layout}"`);
  if (parsed.layoutConfig.layoutId) attrs.push(`layoutId="${parsed.layoutConfig.layoutId}"`);
  ...
  if (Object.keys(parsed.variants).length)
    attrs.push(`variants={${serializeObject(parsed.variants as Record<string, unknown>)}}`);
  if (parsed.variantState.initial) attrs.push(`initial="${parsed.variantState.initial}"`);
  if (parsed.variantState.animate) attrs.push(`animate="${parsed.variantState.animate}"`);
  if (parsed.variantState.exit) attrs.push(`exit="${parsed.variantState.exit}"`);
