...
description: "Integration guides for React (Next.js, Vite), React Native, Vue, and Vanilla JS — plus troubleshooting."
---

motionwind is **multi-framework**. The same `animate-*` class syntax works across React, React
Native, Vue, and plain HTML/JS — each via a dedicated package built on the shared
`motionwind-core` parser.
...
Vue uses [Motion for Vue](https://motion.dev/docs/vue) under the hood. Install both packages:
...
Just like the React Babel plugin, a compiler transform lets you write plain elements with
`animate-*` classes — no per-file imports. Add the transform to `@vitejs/plugin-vue` and register
the component once:
...
Now write plain elements — the transform rewrites them at build time:
...
### Runtime component (no build step)

Prefer not to touch the compiler? Use the `<Motionwind>` component, the `mw.*` proxy, or the
`useMotionwind` composable directly:
...
> Only static `class` attributes are transformed (like the React build step). For dynamic
> `:class`, use `<Motionwind>` / `mw.*`. Scroll-linked (`animate-scroll:*`) is React/vanilla-only
> for now in Vue.
...
For plain HTML with no framework, `motionwind-vanilla` scans the DOM and wires animations via the
[Motion](https://motion.dev) vanilla API.
...
// scans the document; returns a cleanup fn. Options: { observe, respectReducedMotion, root }
const stop = motionwind({ observe: true });
```

### CDN (zero build)

The CDN bundle is self-contained (Motion included) and auto-initializes on load:
...
Supports hover, tap, focus, in-view, enter, and scroll-linked. Honors `prefers-reduced-motion`,
is idempotent across re-runs, and can watch for dynamically-added elements via `{ observe: true }`.

---

## React Native / Expo

See the dedicated [React Native guide](/docs/react-native/installation). In short:
...
Then use the `mw.*` runtime components with the same `animate-*` classes.
