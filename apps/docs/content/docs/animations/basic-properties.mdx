---
title: "Basic Property Animations"
description: "Animate opacity, scale, rotation, translation, dimensions, border radius, colors, shadows, blur, clip path, position, spacing, typography, and more with simple Tailwind-like classes."
---

Motionwind covers the registered Motion property set with the familiar
`animate-{gesture}:{property}-{value}` syntax. React and Vue compile eligible
static tokens; runtime adapters map the same parsed values to their platform.
Check [Compatibility](/docs/compatibility) before using CSS-specific values in
vanilla or React Native code.

This page covers the foundational animatable properties you will use most often.

---

## Opacity

Fade elements in and out by animating their opacity. Values are specified on a 0-100 scale where `0` is fully transparent and `100` is fully opaque.

The parser divides the number by 100, so `opacity-0` becomes `0` and `opacity-100` becomes `1`.

<Demo title="Fade In on Enter">
  <div className="px-8 py-6 bg-[#c8ff2e]/20 border border-[#c8ff2e]/40 rounded-xl text-center font-medium text-[var(--color-fg)] animate-initial:opacity-0 animate-enter:opacity-100 animate-duration-1000">
    I faded in
  </div>
</Demo>

```html
<div
  className="animate-initial:opacity-0 animate-enter:opacity-100 animate-duration-1000"
>
  I faded in
</div>
```

**Compiles to:**

```jsx
<motion.div
  initial={{ opacity: 0 }}
  animate={{ opacity: 1 }}
  transition={{ duration: 1 }}
>
  I faded in
</motion.div>
```

---

## Scale

Scale elements up or down. Values are specified as percentages where `100` equals the original size, `110` is 10% larger, and `50` is half the size.

The parser divides by 100, so `scale-110` becomes `1.1`.

<Demo title="Hover to Scale Up">
  <button className="px-6 py-3 bg-fd-primary text-fd-primary-foreground rounded-xl font-medium cursor-pointer animate-hover:scale-110 animate-spring">
    Hover me
  </button>
</Demo>

```html
<button className="animate-hover:scale-110 animate-spring">Hover me</button>
```

**Compiles to:**

```jsx
<motion.button whileHover={{ scale: 1.1 }} transition={{ type: "spring" }}>
  Hover me
</motion.button>
```

---

## Rotate

Rotate elements by a given number of degrees. Positive values rotate clockwise, negative values rotate counter-clockwise.

<Demo title="Hover to Rotate">
  <div className="w-16 h-16 bg-fd-primary rounded-lg flex items-center justify-center text-fd-primary-foreground font-bold text-xl cursor-pointer animate-hover:rotate-45 animate-spring">
    R
  </div>
</Demo>

```html
<div className="animate-hover:rotate-45 animate-spring">R</div>
```

**Compiles to:**

```jsx
<motion.div whileHover={{ rotate: 45 }} transition={{ type: "spring" }}>
  R
</motion.div>
```

You can also use negative rotation with a `-` prefix:

```html
<div className="animate-hover:-rotate-90">Spin back</div>
```

This compiles to `whileHover={{ rotate: -90 }}`.

---

## Translate X

Slide elements horizontally. The value is in pixels. Positive moves right, negative moves left.

<Demo title="Hover to Slide Right">
  <div className="px-6 py-3 bg-[var(--color-surface-elevated)] border border-[var(--color-border)] rounded-xl text-[var(--color-fg)] font-medium cursor-pointer animate-hover:x-20 animate-spring">
    Slide right
  </div>
</Demo>

```html
<div className="animate-hover:x-20 animate-spring">Slide right</div>
```

**Compiles to:**

```jsx
<motion.div whileHover={{ x: 20 }} transition={{ type: "spring" }}>
  Slide right
</motion.div>
```

For sliding left, use a negative value:

```html
<div className="animate-hover:-x-40">Slide left</div>
```

---

## Translate Y

Slide elements vertically. Positive values move down, negative values move up.

<Demo title="Hover to Float Up">
  <div className="w-20 h-20 bg-fd-primary rounded-2xl flex items-center justify-center text-fd-primary-foreground font-bold cursor-pointer animate-hover:-y-20 animate-spring">
    UP
  </div>
</Demo>

```html
<div className="animate-hover:-y-20 animate-spring">UP</div>
```

**Compiles to:**

```jsx
<motion.div whileHover={{ y: -20 }} transition={{ type: "spring" }}>
  UP
</motion.div>
```

A common pattern is using `y` with `initial` and `inview` for scroll-reveal effects:

```html
<div className="animate-initial:y-40 animate-inview:y-0 animate-once">
  Slides up into view
</div>
```

---

## Width

Animate the width of an element. The value is in pixels by default.

<Demo title="Hover to Expand Width">
  <div className="h-12 w-[100px] bg-fd-primary rounded-xl flex items-center justify-center text-fd-primary-foreground font-medium cursor-pointer animate-hover:w-200 animate-spring">
    Expand
  </div>
</Demo>

```html
<div className="w-[100px] animate-hover:w-200 animate-spring">Expand</div>
```

**Compiles to:**

```jsx
<motion.div whileHover={{ width: 200 }} transition={{ type: "spring" }}>
  Expand
</motion.div>
```

### Unit Values for Width

Width also supports unit suffixes: `pct`, `px`, `vh`, `vw`, `rem`, `em`, and the keyword `auto`.

```html
<!-- Percentage -->
<div className="animate-hover:w-100pct">Full width</div>

<!-- Viewport width -->
<div className="animate-hover:w-50vw">Half viewport</div>

<!-- Auto -->
<div className="animate-hover:w-auto">Auto width</div>
```

**Compiles to:**

```jsx
// w-100pct
whileHover={{ width: "100%" }}

// w-50vw
whileHover={{ width: "50vw" }}

// w-auto
whileHover={{ width: "auto" }}
```

---

## Height

Animate the height of an element. The value is in pixels by default.

<Demo title="Hover to Grow Taller">
  <div className="w-24 h-[48px] bg-[#c8ff2e]/20 border-2 border-[#c8ff2e] rounded-xl flex items-center justify-center text-[var(--color-fg)] font-medium cursor-pointer animate-hover:h-100 animate-spring">
    Grow
  </div>
</Demo>

```html
<div className="h-[48px] animate-hover:h-100 animate-spring">Grow</div>
```

**Compiles to:**

```jsx
<motion.div whileHover={{ height: 100 }} transition={{ type: "spring" }}>
  Grow
</motion.div>
```

### Unit Values for Height

Like width, height supports unit suffixes: `pct`, `px`, `vh`, `vw`, `rem`, `em`, and the keyword `auto`.

```html
<!-- Percentage -->
<div className="animate-hover:h-100pct">Full height</div>

<!-- Viewport height -->
<div className="animate-enter:h-100vh">Full viewport height</div>

<!-- Auto -->
<div className="animate-hover:h-auto">Auto height</div>
```

**Compiles to:**

```jsx
// h-100pct
whileHover={{ height: "100%" }}

// h-100vh
animate={{ height: "100vh" }}

// h-auto
whileHover={{ height: "auto" }}
```

---

## Border Radius

Animate the border radius to morph between shapes. The value is in pixels.

<Demo title="Hover to Morph Shape">
  <div className="w-20 h-20 bg-fd-primary rounded-lg flex items-center justify-center text-fd-primary-foreground font-bold cursor-pointer animate-hover:rounded-40 animate-spring">
    Hi
  </div>
</Demo>

```html
<div className="rounded-lg animate-hover:rounded-40 animate-spring">Hi</div>
```

**Compiles to:**

```jsx
<motion.div whileHover={{ borderRadius: 40 }} transition={{ type: "spring" }}>
  Hi
</motion.div>
```

This is a great way to smoothly transition a square into a circle (or vice versa) on interaction.

---

## Colors

Motionwind provides shorthand classes for the most common color properties, as well as the general-purpose arbitrary value syntax for any CSS property.

### Color Shorthand Classes

Use the `bg-`, `text-`, and `border-` prefixes followed by a literal color value to animate `backgroundColor`, `color`, and `borderColor` respectively.

```html
<button
  className="animate-hover:bg-#c8ff2e animate-hover:text-#000000 animate-duration-300"
>
  Hover for color
</button>
```

**Compiles to:**

```jsx
<motion.button
  whileHover={{ backgroundColor: "#c8ff2e", color: "#000000" }}
  transition={{ duration: 0.3 }}
>
  Hover for color
</motion.button>
```

The supported shorthand mappings are:

| Class            | Motion Property          |
| ---------------- | ------------------------ |
| `bg-{color}`     | `backgroundColor: color` |
| `text-{color}`   | `color: color`           |
| `border-{color}` | `borderColor: color`     |

```html
<!-- Background color -->
<div className="animate-hover:bg-#c8ff2e">...</div>

<!-- Text color -->
<div className="animate-hover:text-#000000">...</div>

<!-- Border color -->
<div className="animate-hover:border-#ff6347">...</div>
```

> **Note:** Color shorthands only work with literal color values starting with `#`, `rgb(`, or `hsl(`. Named colors like `red` or `blue` are not supported -- use the hex or function form instead.

### Arbitrary Value Syntax

For properties that don't have a dedicated shorthand -- like `backgroundColor` -- you can also use the arbitrary value syntax: `[key=value]`.

The value inside the brackets is passed through directly. If it parses as a number, it becomes a number; otherwise it stays a string.

<Demo title="Hover to Change Color">
  <button className="px-6 py-3 bg-[var(--color-surface-elevated)] text-[var(--color-fg)] rounded-xl font-medium cursor-pointer border border-[var(--color-border)] animate-hover:[backgroundColor=#c8ff2e] animate-hover:[color=#000000] animate-duration-300">
    Hover for color
  </button>
</Demo>

```html
<button
  className="animate-hover:[backgroundColor=#c8ff2e] animate-hover:[color=#000000] animate-duration-300"
>
  Hover for color
</button>
```

**Compiles to:**

```jsx
<motion.button
  whileHover={{ backgroundColor: "#c8ff2e", color: "#000000" }}
  transition={{ duration: 0.3 }}
>
  Hover for color
</motion.button>
```

You can use any valid CSS property name and value with the arbitrary syntax:

```html
<!-- Animate padding -->
<div className="animate-hover:[padding=32]">...</div>

<!-- Animate letter spacing -->
<div className="animate-hover:[letterSpacing=4]">...</div>
```

---

## Box Shadow

Animate box shadows using the bracket syntax `shadow-[value]`. Since shadow values contain spaces, use underscores (`_`) in place of spaces within the bracket value.

```html
<div
  className="animate-hover:shadow-[0_20px_60px_rgba(200,255,46,0.3)] animate-duration-300"
>
  Glowing card
</div>
```

**Compiles to:**

```jsx
<motion.div
  whileHover={{ boxShadow: "0 20px 60px rgba(200,255,46,0.3)" }}
  transition={{ duration: 0.3 }}
>
  Glowing card
</motion.div>
```

> **Note:** Spaces in shadow values need underscores. For more complex values, you can also use the arbitrary syntax `[boxShadow=0_20px_60px_rgba(200,255,46,0.3)]`.

You can also remove a shadow on hover by setting it to `none`:

```html
<div className="animate-hover:shadow-[none]">Remove shadow on hover</div>
```

---

## Backdrop Blur

Animate the backdrop blur filter on an element. The value is in pixels and maps to `backdropFilter: "blur(Npx)"`.

```html
<div className="animate-hover:backdrop-blur-10 animate-duration-300">
  Blur the background
</div>
```

**Compiles to:**

```jsx
<motion.div
  whileHover={{ backdropFilter: "blur(10px)" }}
  transition={{ duration: 0.3 }}
>
  Blur the background
</motion.div>
```

A common use is combining backdrop blur with a semi-transparent background for frosted-glass effects:

```html
<div
  className="bg-[var(--color-surface-elevated)] animate-hover:backdrop-blur-20 animate-duration-500"
>
  Frosted glass on hover
</div>
```

---

## Clip Path

Animate the `clipPath` property using the bracket syntax `clip-[value]`. This is useful for reveal effects and creative shape transitions.

```html
<div
  className="animate-initial:clip-[circle(0%)] animate-enter:clip-[circle(50%)] animate-duration-800"
>
  Reveal with clip path
</div>
```

**Compiles to:**

```jsx
<motion.div
  initial={{ clipPath: "circle(0%)" }}
  animate={{ clipPath: "circle(50%)" }}
  transition={{ duration: 0.8 }}
>
  Reveal with clip path
</motion.div>
```

You can use any valid `clip-path` value inside the brackets:

```html
<!-- Polygon -->
<div className="animate-hover:clip-[polygon(50%_0%,100%_100%,0%_100%)]">
  ...
</div>

<!-- Inset -->
<div className="animate-hover:clip-[inset(10%)]">...</div>
```

---

## Position Properties

Animate the CSS position offsets `top`, `left`, `right`, and `bottom`. Values are in pixels by default, and support unit suffixes (`pct`, `px`, `vh`, `vw`, `rem`, `em`) as well as the keyword `auto`.

```html
<div
  className="relative animate-initial:top-100 animate-enter:top-0 animate-duration-500"
>
  Slides down from 100px to 0
</div>
```

**Compiles to:**

```jsx
<motion.div
  initial={{ top: 100 }}
  animate={{ top: 0 }}
  transition={{ duration: 0.5 }}
>
  Slides down from 100px to 0
</motion.div>
```

All four directions are supported:

```html
<!-- Left with percentage -->
<div className="animate-enter:left-50pct">...</div>

<!-- Right -->
<div className="animate-hover:right-20">...</div>

<!-- Bottom with viewport units -->
<div className="animate-hover:bottom-10vh">...</div>
```

**Compiles to:**

```jsx
// left-50pct
animate={{ left: "50%" }}

// right-20
whileHover={{ right: 20 }}

// bottom-10vh
whileHover={{ bottom: "10vh" }}
```

---

## Spacing

Animate `padding`, `margin`, and `gap`. Values are numeric by default, and support unit suffixes (`pct`, `px`, `rem`, `em`).

```html
<div className="animate-hover:p-32 animate-spring">Expand padding on hover</div>
```

**Compiles to:**

```jsx
<motion.div whileHover={{ padding: 32 }} transition={{ type: "spring" }}>
  Expand padding on hover
</motion.div>
```

The supported shorthand mappings are:

| Class     | Motion Property |
| --------- | --------------- |
| `p-{n}`   | `padding: n`    |
| `m-{n}`   | `margin: n`     |
| `gap-{n}` | `gap: n`        |

```html
<!-- Margin -->
<div className="animate-hover:m-16">...</div>

<!-- Gap -->
<div className="animate-hover:gap-24">...</div>

<!-- Padding with rem units -->
<div className="animate-hover:p-2rem">...</div>
```

**Compiles to:**

```jsx
// m-16
whileHover={{ margin: 16 }}

// gap-24
whileHover={{ gap: 24 }}

// p-2rem
whileHover={{ padding: "2rem" }}
```

---

## Typography

Animate font size, letter spacing, and line height. Values are numeric by default, and support unit suffixes (`pct`, `px`, `rem`, `em`).

| Class           | Motion Property    |
| --------------- | ------------------ |
| `text-size-{n}` | `fontSize: n`      |
| `tracking-{n}`  | `letterSpacing: n` |
| `leading-{n}`   | `lineHeight: n`    |

```html
<div
  className="animate-hover:text-size-24 animate-hover:tracking-2 animate-duration-300"
>
  Larger text on hover
</div>
```

**Compiles to:**

```jsx
<motion.div
  whileHover={{ fontSize: 24, letterSpacing: 2 }}
  transition={{ duration: 0.3 }}
>
  Larger text on hover
</motion.div>
```

Line height works the same way:

```html
<div className="animate-hover:leading-32">...</div>
```

**Compiles to:**

```jsx
<motion.div whileHover={{ lineHeight: 32 }}>...</motion.div>
```

---

## Border Width

Animate the border width of an element. The value is in pixels.

```html
<div className="animate-hover:border-w-2 animate-duration-300">
  Border appears on hover
</div>
```

**Compiles to:**

```jsx
<motion.div whileHover={{ borderWidth: 2 }} transition={{ duration: 0.3 }}>
  Border appears on hover
</motion.div>
```

This pairs well with the `border-{color}` shorthand:

```html
<div
  className="animate-hover:border-w-2 animate-hover:border-#c8ff2e animate-duration-300"
>
  Colored border on hover
</div>
```

**Compiles to:**

```jsx
<motion.div
  whileHover={{ borderWidth: 2, borderColor: "#c8ff2e" }}
  transition={{ duration: 0.3 }}
>
  Colored border on hover
</motion.div>
```

---

## Combining Multiple Properties

You can animate multiple properties on the same gesture by stacking classes. Each gesture collects all its property-value pairs into a single object.

<Demo title="Combined Hover Effect">
  <div className="px-8 py-6 bg-[var(--color-surface-elevated)] border border-[var(--color-border)] rounded-xl text-[var(--color-fg)] font-medium cursor-pointer animate-hover:scale-105 animate-hover:-y-4 animate-hover:rounded-20 animate-hover:[backgroundColor=#c8ff2e] animate-hover:[color=#000000] animate-spring">
    Everything at once
  </div>
</Demo>

```html
<div
  className="animate-hover:scale-105 animate-hover:-y-4 animate-hover:rounded-20 animate-hover:[backgroundColor=#c8ff2e] animate-hover:[color=#000000] animate-spring"
>
  Everything at once
</div>
```

**Compiles to:**

```jsx
<motion.div
  whileHover={{
    scale: 1.05,
    y: -4,
    borderRadius: 20,
    backgroundColor: "#c8ff2e",
    color: "#000000",
  }}
  transition={{ type: "spring" }}
>
  Everything at once
</motion.div>
```

All property classes sharing the same gesture prefix are merged into a single Motion prop object at build time.
