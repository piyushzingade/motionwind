...
The classic "draw-on" effect animates `pathLength` from `0` to `1`. Since `pathLength` is not a built-in motionwind property, you use bracket syntax:
...
|---|---|---|
| Path drawing (`pathLength`) | Bracket syntax | `animate-initial:[pathLength=0] animate-enter:[pathLength=1]` |
