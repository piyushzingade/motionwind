...
Layout animations are a Motion feature that automatically animates elements when their position or size changes in the DOM. These are not expressed as motionwind utility classes. Instead, you use Motion's `layout` prop and `layoutId` directly on `motion.*` components.

This page explains each layout animation pattern and shows how it integrates with motionwind classes on surrounding elements.

## Layout prop basics

The `layout` prop tells Motion to automatically animate an element whenever its layout (position or size) changes between renders.
...
When elements reorder in a list, each element's position changes. The `layout` prop animates these position shifts.
    ...
    <>
      <div className="grid grid-cols-3 gap-4">
        {items.map((item) => (
          ...
          >
            <motion.h3 layoutId={`title-${item.id}`} className="font-bold text-white">
              {item.title}
            </motion.h3>
          </motion.div>
        ))}
    ...
    >
      {items.map((item) => (
        <Reorder.Item
          key={item}
          value={item}
          className="cursor-grab active:cursor-grabbing rounded-lg bg-[#1a1a2e] border border-[#c8ff2e]/10 px-4 py-3 text-white select-none"
          whileDrag={{
            scale: 1.03,
      ...
      >
        {items.map((item) => (
          <motion.div
            ...
            layout
            transition={{ type: "spring", stiffness: 300, damping: 25 }}
            className="rounded-xl bg-[#1a1a2e] border border-[#c8ff2e]/10 p-6 text-center text-white font-bold"
          >
            {item}
          </motion.div>
        ))}
...
| Animating DOM position/size changes | Motion `layout` prop directly |
| Shared element transitions | Motion `layoutId` prop directly |
| Drag-to-reorder lists | Motion `Reorder` component directly |
| Grid/list reflow animations | Motion `layout` prop on each item |

Layout animations and motionwind classes serve different purposes and work well together. Use motionwind for gesture-driven and transition-based animations. Use Motion's layout system when you need to animate changes to an element's position or size that result from React state updates or DOM reflows.
