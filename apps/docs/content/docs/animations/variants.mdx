...
Motion's variant system lets you define named animation states and orchestrate parent-child relationships. Motionwind expresses variants declaratively with `animate-variant-*` classes, and you can still drop to Motion's `variants` prop directly for the most advanced cases.

## Variants as classes

Define named states with `animate-variant-{name}:{property}` and select the active state with `animate-from-{name}` (initial), `animate-to-{name}` (animate), and `animate-exit-{name}` (exit):

<Playground
  initial="animate-variant-hidden:opacity-0 animate-variant-hidden:y-20 animate-variant-visible:opacity-100 animate-variant-visible:y-0 animate-from-hidden animate-to-visible animate-duration-500 rounded-xl bg-fd-primary px-6 py-4 text-fd-primary-foreground font-medium"
  text="Named states"
/>

```html
<div
  className="animate-variant-hidden:opacity-0 animate-variant-hidden:y-20 animate-variant-visible:opacity-100 animate-variant-visible:y-0 animate-from-hidden animate-to-visible animate-duration-500"
>
  I animate from hidden → visible
</div>
```
...
A parent's active state propagates to children that declare matching variant names. Add
orchestration with `animate-stagger-{ms}` and `animate-delay-children-{ms}` on the parent —
children only need the matching `animate-variant-*` definitions (no `from`/`to`).

```html
<ul className="animate-to-visible animate-stagger-100">
  <li
    className="animate-variant-hidden:opacity-0 animate-variant-visible:opacity-100"
  >
    One
  </li>
  <li
...
The rest of this page covers the underlying Motion `variants` API, stagger via delay classes, and when to reach for each approach.

## Live Examples

<Demo title="Staggered Card Grid">
  <StaggeredGridDemo />
</Demo>

<Demo title="Multi-State Card">
  <MultiStateDemo />
</Demo>

<Demo title="Collapsible Sidebar">
  <CollapsibleSidebarDemo />
</Demo>

<Demo title="Notification Stack">
  <NotificationStackDemo />
</Demo>

<Demo title="Orchestrated Form">
...
<Demo title="Stagger with delay classes">
  <div className="flex gap-3">
    <div className="animate-initial:opacity-0 animate-initial:y-20 animate-enter:opacity-100 animate-enter:y-0 animate-duration-500 w-12 h-12 rounded-lg bg-fd-primary flex items-center justify-center text-fd-primary-foreground font-bold">
      1
    </div>
    <div className="animate-initial:opacity-0 animate-initial:y-20 animate-enter:opacity-100 animate-enter:y-0 animate-duration-500 animate-delay-100 w-12 h-12 rounded-lg bg-fd-primary/80 flex items-center justify-center text-fd-primary-foreground font-bold">
      2
    </div>
    <div className="animate-initial:opacity-0 animate-initial:y-20 animate-enter:opacity-100 animate-enter:y-0 animate-duration-500 animate-delay-200 w-12 h-12 rounded-lg bg-fd-primary/60 flex items-center justify-center text-fd-primary-foreground font-bold">
      3
    </div>
    <div className="animate-initial:opacity-0 animate-initial:y-20 animate-enter:opacity-100 animate-enter:y-0 animate-duration-500 animate-delay-300 w-12 h-12 rounded-lg bg-fd-primary/40 flex items-center justify-center text-fd-primary-foreground font-bold">
      4
    </div>
  </div>
</Demo>
...
<div className="flex gap-3">
  <div
    className="animate-initial:opacity-0 animate-initial:y-20 animate-enter:opacity-100 animate-enter:y-0 animate-duration-500"
  >
    1
  </div>
  <div
    className="animate-initial:opacity-0 animate-initial:y-20 animate-enter:opacity-100 animate-enter:y-0 animate-duration-500 animate-delay-100"
  >
    2
  </div>
  <div
    className="animate-initial:opacity-0 animate-initial:y-20 animate-enter:opacity-100 animate-enter:y-0 animate-duration-500 animate-delay-200"
  >
    3
  </div>
  <div
    className="animate-initial:opacity-0 animate-initial:y-20 animate-enter:opacity-100 animate-enter:y-0 animate-duration-500 animate-delay-300"
  >
    4
  </div>
</div>
...
Motionwind's compiler transforms statically discoverable animation tokens. When
the animation token or variant value itself depends on runtime state, use the
`mw.*` component, which parses Motionwind classes at runtime.
    ...
    >
      {["Home", "Search", "Settings"].map((label) => (
        ...
        >
          ...
          <motion.span variants={labelVariants}>{label}</motion.span>
        </motion.div>
      ))}
    </motion.nav>
  );
